...
#' @export
ITROC_emp <- function(p1, Xlist){
  t1 <- quantile(x = Xlist[[1]], probs = 1 - p1, names = FALSE)
  Fn <- lapply(Xlist[-1], function(x) ecdf(x))
  all_Se <- lapply(Fn, function(ff) ff(t1))
  res <- Reduce("*", all_Se)
  return(res)
}

#' @export
TAUC_emp <- function(X_list){
  ## X_list: list of X1, X2, ..., Xk
  xx <- X_list[[1]]
  all_X_d <- expand.grid(X_list[-1])
  min_X_d <- apply(all_X_d, 1, min)
  res <- mean(sapply(xx, function(x) mean(x < min_X_d)))
  return(res)
}

#' @export
ITFNR_emp <- function(X_list){
  ## X_list: list of X1, X2, ..., Xk
  xx <- X_list[[1]]
  all_X_d <- expand.grid(X_list[-1])
  max_X_d <- apply(all_X_d, 1, max)
  res <- mean(sapply(xx, function(x) mean(x >= max_X_d)))
  return(res)
}
